...
import { CopyToDirOperationStage } from "../../defines"
import {
	FileManagerCopyConflictError,
	FileManagerCopyOperationError,
} from "../../error"
import { CopyToDirOperationRunner } from "../copyToDirOperationRunner"
...
describe("CopyToDirOperationRunner", () => {
	test("copies empty array successfully", async () => {
		...
		expect(state.stage).toBe(CopyToDirOperationStage.COPYING)
	...
	test("copies input handles into output directory", async () => {
		const fs = new InMemoryFs()
		const store = createStore()
		const runner = new CopyToDirOperationRunner(store)
		const root = await fs.getRootDir()
		...
		const writer = await inputFile.write({ mode: FsWriteMode.OVERWRITE })
		await writer.write(new Blob(["abc"], { type: "text/plain" }))
		await writer.close()
		...
		if (state.stage === CopyToDirOperationStage.COPYING) {
			expect(state.totalFilesProcessed).toBe(1)
			expect(state.totalDirsProcessed).toBe(0)
			expect(state.totalBytesCopied).toBe(3)
			expect(state.totalFilesSkipped).toBe(0)
		...
		const content = await Blobs.blobToText(await copied.getFile())
		expect(content).toBe("abc")
	})

	test("interrupts operation when interrupt atom is triggered", async () => {
		const fs = new InMemoryFs()
		const store = createStore()
		const runner = new CopyToDirOperationRunner(store)
		const root = await fs.getRootDir()
	...
	test("counts skipped files when resolver skips", async () => {
		const fs = new InMemoryFs()
		const store = createStore()
		const runner = new CopyToDirOperationRunner(store)
		const root = await fs.getRootDir()
		...
		await writeFile("existing", "out/input.txt", fs)
		...
		if (state.stage === CopyToDirOperationStage.COPYING) {
			...
			expect(state.totalFilesProcessed).toBe(0)
			expect(state.totalBytesCopied).toBe(0)
		}
	})

	test("counts skipped directories when conflict resolver skips", async () => {
		const fs = new InMemoryFs()
		const store = createStore()
		const runner = new CopyToDirOperationRunner(store)
		const root = await fs.getRootDir()
		const inputDir = await root.openDir(Path.fromSegment("input"), {
			create: true,
		})
		const outputDir = await root.openDir(Path.fromSegment("out"), {
			create: true,
		})
		const blockingFile = await outputDir.openFile(Path.parse("input"), {
			create: true,
		})
		const writer = await blockingFile.write({ mode: FsWriteMode.OVERWRITE })
		await writer.write(new Blob(["block"], { type: "text/plain" }))
		await writer.close()
		...
		expect(state.stage).toBe(CopyToDirOperationStage.COPYING)
		if (state.stage === CopyToDirOperationStage.COPYING) {
			expect(state.totalDirsSkipped).toBe(1)
			expect(state.totalDirsProcessed).toBe(1)
		}
	})

	test("stores error when conflict resolver reports error", async () => {
		const fs = new InMemoryFs()
		const store = createStore()
		const runner = new CopyToDirOperationRunner(store)
		const root = await fs.getRootDir()
